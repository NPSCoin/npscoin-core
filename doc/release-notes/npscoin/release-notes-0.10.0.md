...
- Darksend tested with 2.5 NPS successfully and I've set that as the minimum
...
- Fixed the endless splitting issue causes by splitting 1000NPS and not making a DS compatible input
...
- Fixed issue with denominating small amounts of NPS in large wallet
  (http://jira.npscoin.qa/browse/NPS-46)
- Made splitting up initial inputs much more efficient. Now when splitting up,
  it will use powers of two from 4096 NPS in reverse to get the best possible
...
- Fixed issue with completed amount jumping around
  (http://jira.npscoin.qa/browse/NPS-46)
- Made messages less threatening (http://jira.npscoin.qa/browse/NPS-60)
...
http://test.explorer.npscoin.fr/tx/ce0ea2bdf630233955d459489b6f764e0d0bbe9e8a62531dd2a14b455626b59c
...
- Client now prioritises sending denominated funds back into Darksend. This will
  improve anonymity and help to respect the "anonymize npscoin" amount in the
  configuration.
- fixed a bug where masternodes send failed transactions
- changed max to 100k in configuration
- added a warning message to startup (delete ~/.npscoin/.agree_to_tou to see it)
...
http://test.explorer.npscoin.fr/tx/6de2c5204abdea451da930f61bae0f954eef13188a3a37a572a24c9d92057d5d
...
- Darksend limited to 5000NPS per wallet.dat. Client will warn about this the
  first time it's opened, then disable darksend from then on.
- Fixed some bugs with connecting to the correct masternodes
- Send was sending way too many coins for all modes, (I sent 100NPS anon and it
  sent 2000NPS, then sent me change for the rest causing a whole reprocess of
...
- Added a pre-session state where the client will query a random masternode
  and ask if they can perform a merge on N npscoin without giving any other
...
http://test.explorer.npscoin.fr/tx/072ca56cbf705b87749513a2d2ee02080d506adcf8fe178f6dc2967f0711788e
http://test.explorer.npscoin.fr/tx/32daa8ca46462e7e99f3532251d68a8c3835a080c937bd83b11db74e47b770ff
...
- Fees per round of Darksend are 0.001NPS or $0.00538 at current prices. This
  means to anonymize 1000NPS with 3 rounds (an average use case) it would cost
  a user 1.5 cents.
...
- RPC calls are changed a bit:
    ...
    darksend denominate 100 - Will denominate 100NPS
    darksend auto - Will run AutoDenominate
    darksend Xaddr 100 - Will send 100 denominated NPS to Xaddr
    sendtoaddress Xaddr 100 - Will send 100 non-denominated NPS to Xaddr
...
/npscoin-qt -datadir=/home/user/.npscoin -listen=0 -darksendrounds=8 -usemasternode="192.168.56.102:19855"
/npscoin-qt -datadir=/home/user/.npscoin2 -listen=0 -darksendrounds=8 -usemasternode="192.168.56.102:19855"

and even disable darksend auto-denom if wanted:
/npscoin-qt -datadir=/home/user/.npscoin -listen=0 -darksendrounds=8 -usemasternode="192.168.56.102:19855" -disabledarksend=1


0.10.12.7 Release notes
-----------------------

- Added a smart input splitting method. Place 1000+NPS into a brand new wallet
...
- Mixing rounds are limited to 1000NPS, per block
...
- DoAutomaticDenominations 100NPS limit changed to 500NPS (we should see a bunch
...
- resolves issue with wallet not syncing by adding the capability to retrieve nodes through dnsseed
   (see https://github.com/npscoinproject/npscoin/pull/21)
- Linux 32 and Mac OS X are now officially supported platforms
   (see https://github.com/npscoinproject/npscoin/pull/17)
- improved overall unit test code coverage
   (see https://github.com/npscoinproject/npscoin/pull/13 and https://github.com/npscoinproject/npscoin/pull/15)
- minor documentation updates
   (https://github.com/npscoinproject/npscoin/pull/18)
