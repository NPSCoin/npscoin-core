...
#if defined(NDEBUG)
# error "NPSCoin Core cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "NPSCoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    CAmount nFounderReward = GetFounderPayment(nPrevHeight + 1);

    if (nPrevHeight == 0) {
        nSubsidy = 60000 * COIN; // Premine 60,000 NPS
    } else if (nPrevHeight >= 1 && nPrevHeight <= 5039) {
        nSubsidy = 8 * COIN; // Instamine Protection For A Week (8 NPS/Block)
    }

    // yearly decline of production by 50% per year, projected ~12M coins max by year 2034+.
    if(nPrevHeight >= consensusParams.nSubsidyHalvingInterval * 4) {
        nSubsidy = 1 * COIN; // Minimum Block Reward
    } else {
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // NPS : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(NPS): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(NPS): couldn't find masternode or superblock payments"),
                                REJECT_INVALID, "bad-cb-payee");
    }
    // END NPS

    // NPS FOUNDER REWARD
    CAmount founderReward = GetFounderPayment(pindex->nHeight);
    if (founderReward > 0) {
        CScript FOUNDER_SCRIPT = GetScriptForDestination(CBitcoinAddress(Params().FounderAddress()).Get());
        bool fFounderPaid = false;
        ...
        if (!fFounderPaid) {
            return state.DoS(0, error("ConnectBlock(NPS): couldn't find founder reward payments"), REJECT_INVALID, "bad-founder-payment");
        }
    }
    // END NPS FOUNDER REWARD
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // NPS : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(NPS): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(NPS): spork is off, skipping transaction locking checks\n");
    }
