...
const char * const BITCOIN_CONF_FILENAME = "npscoin.conf";
const char * const BITCOIN_PID_FILENAME = "npscoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "npscoin" is a composite category enabling all NPSCoin-related debug output
            if(ptrCategory->count(string("npscoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NPSCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NPSCoin
    // Mac: ~/Library/Application Support/NPSCoin
    // Unix: ~/.npscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NPSCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NPSCoin";
#else
    // Unix
    return pathRet / ".npscoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty npscoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override npscoin.conf
